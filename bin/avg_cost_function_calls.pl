...
foreach my $data_set (@DATA_SETS)
{
  ...
  {
    my $avg_calls_of_cost_function = 0;
    foreach my $i (1..$m)
    {
      ...
      while (<LOG>)
      {
        if ($_ =~ /Number.*subsets:\s(\d+)/)
        {
          $avg_calls_of_cost_function += $1;
        }
      }
      close (LOG);
    }

    $avg_calls_of_cost_function /= $m;
    printf DATA "%2d runs on %14s with %8s calculates ". 
    "the cost function %6d times on average.\n",
    $m, $data_set, $algorithm, $avg_calls_of_cost_function;

    printf "%2d runs on %14s with %8s calculates ". 
    "the cost function %6d times on average.\n",
    $m, $data_set, $algorithm, $avg_calls_of_cost_function;
  }
}
