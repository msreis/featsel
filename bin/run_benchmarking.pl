...
if ($instance_mode == 0)
{
  foreach my $i (1..$maximum_instance_size)    
  {
    ...
    foreach (1..$number_of_instances_per_size)
    {
      ...
      elsif ($cost_function eq 'MutualInformation')
      {
        mi::random_MutualInformation_instance
        ($i, $MAX_ELEM_VALUE, $INPUT_DIR, $_);
      }
      elsif ($cost_function eq 'ConditionalMutualInformation')
      {
        cmi::random_ConditionalMutualInformation_instance
        ($i, $MAX_ELEM_VALUE, $INPUT_DIR, $_);
      }
