...
my @ALGORITHMS = (
  ...
  # "sffs",
  "all");
my %cost_function = ("pucs" => "mce", "sffs" => "mce", "es" => "mce", 
                     "ucs" => "mce", "bfs" => "mce", "chcga" => "mce");
my @DATA_SETS = (
                 #"Iris",
                 "Breast_cancer",
                 "Wine",
...
foreach my $data_set (@DATA_SETS)
{
  foreach my $algorithm (@ALGORITHMS)
  {
    ...
    foreach my $i (1..$m)
    {
      ...
      if (($algorithm eq "es" and $features{$data_set} > 22) or
          ($algorithm eq "ucs" and $features{$data_set} > 22) or
          ($algorithm eq "ucsr" and $features{$data_set} > 22))
      {
        ...
        {
          my $t;
          if ($data_set eq "HillValley")
          {
            $t = 5000;
          }
          if ($data_set eq "Lung_cancer")
          {
            $t = 10000;
          }
          $t0 = [gettimeofday];
          ...
          my $featsel_call_string = "$FEATSEL_BIN " .
             "-n $features{$data_set} -l $labels{$data_set} " .
             "-c $cost_function{$algorithm} -f $dat_file{$data_set} " .
             "-a $algorithm -t $t > $LOG_FILE";
