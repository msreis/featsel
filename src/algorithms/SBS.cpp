//
// SBS.cpp -- implementation of the class "SBS".
//
...
void SBS::get_minima_list (unsigned int max_size_of_minima_list)
{
  ...
  do
  {
    ...
    // if k = |S| then there is no more element to remove from X in order to
    // decrease the value of c(X)
    //
  }
  while ((X.get_subset_cardinality () > 0) &&
         (k < set->get_set_cardinality ()) &&
