...
#include "functions/mi.h"
#include "functions/cmi.h"
...
int main(int argc, char * argv[])
{
  ...
  else if (algorithm.compare ("SpecCMI") == 0)
    solver = new spec_cmi ();
  else
    return EXIT_FAILURE;
  ...
  else if (a_cost_function.compare ("MutualInformation") == 0)
    cost_function = new mi (S);
  else if (a_cost_function.compare ("ConditionalMutualInformation") == 0)
    cost_function = new cmi (S);
  else
    return EXIT_FAILURE;
  ...
  for (i = 1; i < argc; i++)
  {
    ...
    else if ( (strcmp (argv[i],"-c") == 0) && ((i+1) < argc) )
    {
      ...
      if ( (0) 
          ...
          || (strcmp (argv[i], "MutualInformation") == 0)
          || (strcmp (argv[i], "ConditionalMutualInformation") == 0)
         )
      {
        c->clear();
        c->append(argv[i]);
