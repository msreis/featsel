// MutualInformation.cpp automatically generated by bin/add_new_cost_function.pl
...
{
  float Pr_X_is_x, MCE = 0, H_X = 0;
  map <string, ElementSubset *>::iterator it;
  float X_card = X->get_subset_cardinality ();
  ElementSet S ("S", X_card, 1);
  float log_2tothecard_of_X = log ((float) pow (2, X_card));
  // calculates the distribution of X = x and initialize samples
  // variable
  calculate_distributions_from_the_samples (& S, X);

  // calculates the MCE and H(X) and clear the table of distribution of
  ...
  for (it = samples.begin (); it != samples.end (); it++)
  {
    if (it->second->cost > 1)
    {
      ...
      MCE += Pr_X_is_x * calculate_conditional_entropy (it->second);
      // H(X) -= Pr(X=x) * log (2^|X|, Pr(X = x))
      H_X -= Pr_X_is_x * (log (Pr_X_is_x) / log_2tothecard_of_X);
    }
    else
...
float MutualInformation::calculate_conditional_entropy (ElementSubset * x)
{
  unsigned int y;
...
void MutualInformation::calculate_distributions_from_the_samples
